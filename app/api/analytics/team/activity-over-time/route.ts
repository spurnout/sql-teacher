...
import { getTeamActivity } from "@/lib/analytics/team-queries";
...
export async function GET(req: NextRequest) {
  ...
  const weeks = Math.min(Math.max(Number(req.nextUrl.searchParams.get("weeks") ?? "12"), 4), 52);
  const members = await getOrgMembers(org.id);
  const memberIds = members.map((m) => m.userId);
  const result = await getTeamActivity(memberIds, weeks);
